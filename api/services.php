...
<body>
    ...
    <div class="container">
        <?php
        $categorias = ['Sites', 'Mobile', 'Design']; // Definir as categorias na ordem desejada

        foreach ($categorias as $categoria) {
            // Consulta SQL para recuperar serviços por categoria e unir com o nome do desenvolvedor
            $query = "
            SELECT s.*, d.nome AS dev_nome 
            FROM tb_cad_servico_dev s
            JOIN tb_cadastro_developer d ON s.id_developer = d.id
            WHERE s.categoria = ? AND s.status != 'pausado'
        ";
            ...
            // Verificar se há serviços para essa categoria
            if ($result->num_rows > 0) {
                echo '<h3>Desenvolvedores de ' . htmlspecialchars($categoria) . ':</h3>';
                echo '<div id="products" class="row list-group">';

                while ($row = $result->fetch_assoc()) {
                    // Exibir cada serviço com o nome do desenvolvedor
        ?>
                    <div class="item col-xs-12 col-sm-6 col-md-4 mb-4">
                        <div class="card shadow-sm h-100">
                            ...
                            <div class="card-body d-flex flex-column">
                                ...
                                <p class="developer text-center mt-3 mb-0">Desenvolvido por: <a href="favorito.php?developer_id=<?= $row['id_developer'] ?>">
                                        <?= htmlspecialchars($row['dev_nome']) ?>
                                    </a></p>
                            </div>
                        </div>
                    </div>
        <?php
                ...
                echo '</div>'; // Fecha div #products
            } else {
                echo '<h3>Não há serviços disponíveis de ' . htmlspecialchars($categoria) . '</h3>';
            }
